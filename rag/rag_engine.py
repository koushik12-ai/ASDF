...
# Integrates LangChain, FAISS vector store, and Groq LLM for RAG-based question answering.
import os
import warnings
...
class RAGReasoningEngine:
    ...
    def load_knowledge_base(self):
        """Returns sample knowledge base documents."""
        ...
        return [Document(page_content=text) for text in faq_data + kb_data]

    def build_vector_index(self):
        """Indexes the knowledge base into FAISS and builds the RAG chain."""
    ...
    def structure_query(self, asr_text: str) -> str:
        """Rewrites a raw ASR transcript into a clean search query."""
        print(f"--> Raw ASR Input: '{asr_text}'")
        clean_prompt = ChatPromptTemplate.from_template(
            "Rewrite the following spoken sentence into a concise search query. "
            "Remove filler words.\nSentence: {text}\nQuery:"
        )
    ...
    def process_request(self, asr_input: str) -> str:
        """End-to-end: clean query → retrieve context → generate response."""
        clean_query = self.structure_query(asr_input)
        print("--> Retrieving context and generating response...")
        return self.rag_chain.invoke(clean_query)
